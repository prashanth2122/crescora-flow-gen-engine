...
| Routine service completion | Routine journeys save/update records, send the booking confirmation, schedule reminders or audits when relevant, and end without queue or handover nodes. |
...
| Appointment patient lookup | Appointment booking starts with a clean Mobile Number field, keeps the Indian-number validation and user-friendly invalid-number handling, requires a 6-digit OTP gate before record fetch, allows up to 3 invalid-format retries, reuses an existing patient profile when found, and asks only remaining required details for a new patient. The new-patient branch assigns the stable `PAT-{{patient_mobile}}` identifier before the required appointment commit, so an empty patient catalog cannot cause a false slot-conflict fallback. |
| Email OTP enforcement | The single-branch appointment variant sends the same OTP through the runtime email adapter to the configured global `{{otp_notification_email}}` (`prashanth.chinala@gmail.com`) and through the approved WhatsApp test template `jaspers_market_order_confirmation_v1` before patient lookup, validates the exact 6-digit value instead of only checking the format, and stops after 3 failed attempts with hospital call guidance. Confirmation and reminders do not use email. |
| Email OTP production validation | Configure the runtime email provider for `prashanth.chinala@gmail.com`, verify the OTP delivery, and confirm expiry, resend cooldown, and attempt limits. The provider test must use the configured recipient, not dummy data. |
| Active appointment check | After email OTP, appointments are looked up by `patient_id`; only `booked`/`confirmed` rows with `scheduled_start_at > now` in `Asia/Kolkata` are shown. Patients see the date and time in plain Indian 12-hour format without timezone offsets. Cancelled, completed, expired, and past same-day rows are excluded, followed by proceed/keep-existing choices. |
...
| Single-branch booking variant | The appointment-only single-branch variant must start with only `📅 Book appointment`, must not prompt for branch or consultation-mode choice, must load DB-backed departments before doctors, and must keep all booking reads/writes scoped to `chanda_nagar`. |
| New-patient form UX | The new-patient form displays only patient name, age, and gender, with gender labels `Male` and `Female`; email is not displayed or required for booking. |
...
| OTP gate behavior | The appointment path uses the native email OTP node before patient lookup with six digits, five-minute expiry, 30-second resend cooldown, three resends, and three attempts; the OTP is not stored in ordinary variables. |
...
| Appointment reminder schedule | After confirmed booking, the single-branch appointment variant schedules two one-shot WhatsApp reminder jobs from `{{appointment_scheduled_at}}`, the normalized appointment start equivalent to `{{appointmentBooking.start}}`: one 12 hours before and one 2 hours before the visit, both using the approved `appointment_reminder` template. |
...
| Delivery status | The active path sends the OTP by email to `{{otp_notification_email}}` and WhatsApp through `jaspers_market_order_confirmation_v1`; appointment confirmation uses `appointment_confirmed`, and reminders use `appointment_reminder`. The approved confirmation template maps patient name, appointment date, appointment time, department, and appointment ID; the reminder template maps appointment date and time. After the synchronous booking commit, the active path executes the confirmation notification before creating reminders and writing the audit event. A notification delivery failure remains internal, follows the default path, and does not suppress the confirmation carousel or appear in the patient transcript. No dummy appointment data is used. |
| Booking response latency | The patient-facing path must complete the reservation hold, one atomic `booking_commit` transaction, and the outbox enqueue before returning the confirmation carousel. The transaction combines duplicate protection, appointment/reservation/payment persistence, and the domain mutation audit; WhatsApp confirmation, reminder-job creation, and secondary audit delivery run from the background outbox route. The outbox worker must claim `healthcare.outbox_events`, re-enter the flow with `appointment_post_booking_tasks`, and use the exact approved WhatsApp template locale. |
